...
- **Prose marks (#223):** `{: .aside}`, `{: .pull-quote}`, `{: .caption}`, `{: .figure-wide}` live in `main.css` for ordinary posts/pages. Do not reuse editorial-grid `.aside` for blog sidenotes. `.figure-wide` must not use `100vw` — that inflates iOS Safari’s layout viewport. It grows by opening the main column to site-main width (`:has(.figure-wide)`) and using `%` + container-padding cancel.
