...
- **Prose marks (#223):** `{: .aside}`, `{: .pull-quote}`, `{: .caption}`, `{: .figure-wide}` live in `main.css` for ordinary posts/pages. Do not reuse editorial-grid `.aside` for blog sidenotes. Sidenotes hang only from `70em` (14.5rem rail needs that leftover). `.figure-wide` uses `100vw` above `40em` and cancels `.container` padding on small screens so it cannot widen the page.
