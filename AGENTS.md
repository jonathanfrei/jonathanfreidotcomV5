...
- **Prose marks (#223):** `{: .aside}`, `{: .pull-quote}`, `{: .caption}`, `{: .figure-wide}` live in `main.css` for ordinary posts/pages. Do not reuse editorial-grid `.aside` for blog sidenotes. Sidenotes hang only from `70em` (14.5rem rail needs that leftover). `.figure-wide` defaults to canceling `.container` padding; `100vw` breakout is `min-width: 40.01em` only. Do not put `overflow-x` on `html` — iOS Safari then sizes the layout viewport to the content and every page looks desktop. Phone layout also keys off `max-device-width` and `html.is-narrow` (set from `screen.width` / iPhone UA) because Safari can keep a ~980px *layout* viewport after `width=device-width`.
